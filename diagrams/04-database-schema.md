...
Система использует **две отдельные базы данных** для изоляции и независимого масштабирования:
...
erDiagram
    ...
    RENTALS {
        string id PK "UUID (копия из db-rental)"
        string user_id "ID пользователя"
        int price_per_hour "Цена за час"
        int free_period_min "Бесплатный период"
...
#### rentals (db-rental)
**Назначение:** Основная таблица аренд в rental-core
...
#### rentals (db-billing)
**Назначение:** Копия аренд для биллинга

**Отличия от db-rental:**
- Нет `powerbank_id`, `deposit` (не нужны для биллинга)
- Синхронизируется из db-rental при создании аренды
- Используется billing-worker для начислений
...
**Как работает:**
1. rental-core создает аренду в `db-rental`
2. rental-core создает копию в `db-billing` (только нужные поля)
3. billing-worker работает только с `db-billing`

**Преимущества:**
- Изоляция: падение одной БД не влияет на другую
- Масштабирование: можно масштабировать БД независимо
- Производительность: billing-worker не нагружает db-rental

---

## Размер данных (Z = 1 KB)

**Одна аренда в db-rental:**
```
rentals:           ~200 bytes
quotes:            ~200 bytes (удаляется после старта)
idempotency_keys:  ~300 bytes
-----------------------------------
Итого db-rental:   ~400-500 bytes
```

**Одна аренда в db-billing:**
```
rentals:           ~150 bytes (меньше полей)
payment_attempts:  ~100 bytes * N попыток
debts:             ~100 bytes (если есть)
-----------------------------------
Итого db-billing:  ~350-450 bytes + N * 100 bytes
```

**При X = 10 RPS:**
- За час: 36,000 аренд × 1 KB ≈ 36 MB (обе БД вместе)
- За сутки: 864,000 аренд × 1 KB ≈ 864 MB
- За месяц: ~25 GB

**Распределение:**
- db-rental: ~10 GB/месяц (меньше данных, больше write)
- db-billing: ~15 GB/месяц (больше данных из-за payment_attempts)

**Оптимизация:**
- Архивирование старых аренд (> 30 дней)
- Удаление payment_attempts после завершения
- Очистка idempotency_keys в db-rental
