...
### Position B — Два сервиса: `rental-core` + `billing-worker` с двумя БД (текущее решение)

**Идея.**  
Разделить систему на два микросервиса и физически разделить данные в двух PostgreSQL-базах:
...
- `rental-core` — HTTP-API и состояние аренды:
  ...
  - читает и пишет данные аренды в БД `db-rental` (офферы, аренды, идемпотентность).
- `billing-worker` — фоновый воркер:
  ...
  - пишет денежные операции и долги в БД `db-billing`.

Каждый сервис имеет доступ к обеим БД через общий слой `shared`, но:

- `db-rental` — основной источник истины про аренды и офферы; в неё пишет в основном `rental-core`;
- `db-billing` — источник истины про платежи и долги; в неё пишет `billing-worker` (для `rental-core` эта БД read-only).
...
- Изоляция денежных данных:
  - тяжёлые таблицы (`payment_attempts`, `debts`) вынесены в отдельную БД;
  - нагрузка от биллинга меньше влияет на онлайн-операции `/rentals/*`.
...
- Инфраструктура остаётся простой для учебного проекта:
  ...
  - достаточно `docker-compose` с двумя сервисами и двумя PostgreSQL-БД.
- Хороший баланс между соответствием NFR (maintainability, reliability, scalability) и сложностью реализации.

**Минусы и ограничения.**

- Общий слой моделей и миграций (`shared`) для обеих БД:
  - требуется дисциплина, чтобы не протаскивать «лишние» таблицы в каждую БД;
  - миграции нужно внимательно поддерживать в двух контурах (`migrations_rental`, `migrations_billing`).
...
- **Инфраструктура.**
  - B: требуется только два сервиса и две PostgreSQL-БД (`db-rental`, `db-billing`) → всё поднимается через `docker-compose`, без отдельного брокера сообщений.
...
- **Сложность разработки.**
  - B: взаимодействие через общие данные в БД (shared storage), без отдельного брокера сообщений — меньше концепций для команды.
